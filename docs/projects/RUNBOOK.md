...
- `AUTO_PORTS=true` (escolhe a próxima porta livre automaticamente)
...
- `KAFKA_REQUIRED=true` (falha se Kafka não estiver disponível com o worker ativo)
...
| Sintoma | Causa comum | Diagnóstico | Ação |
|---|---|---|---|
...
| `EACCES` em `dist`, `.next`, `node_modules` | Permissão herdada de container | `ls -ld <pasta>` | Reexecutar `./start-dev.sh` (autocorreção) ou ajustar ownership manual |
| Antifraude não sobe após migrate | DB não pronto ou migration pendente | `docker compose -f docker-compose.infra.yaml ps` e logs do processo | Subir infra novamente e repetir `./start-dev.sh` |
| Warning de orphan containers | Containers antigos no projeto | `docker compose ps -a` | `docker compose down --remove-orphans` |
| Kafka fora do ar | Infra não iniciada ou broker indisponível | `docker compose -f docker-compose.infra.yaml ps` | `docker compose -f docker-compose.infra.yaml up -d` |
